...
class BooleanQuery extends _Query.default {
  ...
  async search(indexer) {
    ...
    switch (this.operator) {
      case "AND":
        documents = documentLists.reduce((acc, list) => acc.filter(doc => list.includes(doc)));
        break;
      case "OR":
        documents = [...new Set(documentLists.flat())];
        break;
      case "NOT":
        documents = documentLists[0].filter(doc => !documentLists.slice(1).flat().includes(doc));
        break;
      default:
        throw new Error(`Invalid operator: ${this.operator}`);
    }
    let scores = this.combineScores(hitsList, documents, this.operator);
    return new _Hits.default(documents.length, documents, scores);
  }
