...
class TermQuery {
  ...
  async search(indexer) {
    const {
      ...
      frequencies,
      doc_freqs
    } = await indexer.database.search(this.term.text);
    const documents = await Promise.all(ids.map(id => indexer.getDocument(id)));
    ...
    return new _Hits.default(Object.keys(tf_idfs).length, filteredDocuments, tf_idfs);
  }
}
